...
public interface AppointmentsRepository extends CrudRepository<Appointments,Integer> {
    //@Query(value="SELECT a.appt_date,a.complaint,d.doctor_name FROM Appointments a INNER JOIN a.Doctors d WHERE a.doctors_id = :doctorId",nativeQuery = true)
    //@Query(value="SELECT * FROM Appointments a JOIN Doctors d ON a.doctors_id = d.id WHERE d.id = :doctorId",nativeQuery = true)
      List<Appointments> findByDoctorsId(Integer doctorId);
      List<Appointments> findByPatientsId(Integer patientId);
